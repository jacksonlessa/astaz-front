# Briefing — Transfer para o Beto Carrero World

**URL**: `/destinos/beto-carrero`
**Intenção de busca**: famílias e grupos hospedados em BC que vão ao parque, em
Penha
**Status**: ⬜ vazio
**Prioridade**: 🔴 alta, com pico em férias escolares e feriados prolongados

---

## 1. Quem chega nesta página

Quase sempre uma família com crianças, ou um grupo de amigos, hospedada em BC.
A decisão real que ela está tomando não é "qual transfer" — é **"vou de carro
ou contrato alguém?"**. A página precisa ganhar essa comparação.

**O que costuma pesar mais na decisão:** estacionamento? cansaço da volta?
não querer dirigir depois de um dia inteiro no parque?

>

## 2. Dados operacionais (obrigatório)

| Dado | Resposta |
| --- | --- |
| Distância BC ↔ Beto Carrero (Penha) | |
| Tempo médio de trajeto | |
| Tempo em alta temporada | |
| Faixa de preço (ida e volta) | |
| Antecedência mínima para agendar | |

## 3. Como funciona a espera durante o dia (obrigatório)

**Este é o ponto que decide a venda** — e provavelmente a pergunta que mais
chega no WhatsApp.

**O motorista fica esperando no parque o dia todo, ou volta e retorna depois?**

>

**Se fica: isso é cobrado por hora? Está incluso no valor da ida e volta?**

>

**Como o cliente avisa que quer voltar antes do horário combinado?**

>

**Qual o horário-limite de retorno?** (o parque fecha tarde em alta temporada)

>

## 4. Particularidades que só vocês sabem

- Melhor horário para sair de BC e evitar fila na entrada do parque?
- O acesso a Penha muda muito em feriado?
- Onde é o ponto de embarque/desembarque no parque?

>

## 5. Perguntas frequentes reais

Mínimo de 3.

1. **P:**
   **R:**

2. **P:**
   **R:**

3. **P:**
   **R:**

## 6. Veículos e capacidade

Grupo grande é comum aqui — a van tende a ser o veículo principal desta página.

| Veículo | Passageiros | Observação |
| --- | --- | --- |
| SUV Premium | | |
| Van Executiva | | |

**Tem cadeirinha e bebê conforto? Para quais idades?**

> **Este campo é decisivo.** Família com criança pequena não fecha sem essa
> resposta, e é um diferencial concreto contra app de corrida.

## 7. Fotos disponíveis

>

## 8. O que NÃO dizer

Cuidado com uso de marca de terceiro: o nome "Beto Carrero World" pode ser
citado como destino, mas o texto não pode sugerir vínculo, parceria ou
representação oficial do parque.

>

---

## Checklist antes de publicar

- [ ] Seções 2, 3 e 5 preenchidas (bloqueiam a publicação)
- [ ] Resposta sobre cadeirinha preenchida
- [ ] Penha está em `areaServed` (`src/lib/site.ts`) ✅ já adicionada
- [ ] Nenhum número no texto final sem origem aqui
